...
const UserListPage = () => {
    ...
    const fetchUsers = async () => {
        setIsLoading(true);
        try {
            const response = await api.get('/users'); // Asumsi endpoint untuk daftar pengguna
        ...
        { header: 'Role', accessor: 'role', sortable: true },
        {
            ...
            render: (row) => (
                <div className="flex space-x-2">
                    <button onClick={() => navigate(`/users/edit/${row.id_user}`)} className="bg-yellow-500 hover:bg-yellow-700 text-white py-1 px-3 rounded text-sm">Edit</button>
                    {currentUser?.id_user !== row.id_user && ( // Tidak bisa menghapus diri sendiri
